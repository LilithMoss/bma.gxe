    ...
    if(length(Cov)>0){
      if(nrow(Sample)!=nrow(Cov)){
        ...
        if(colnames(Sample)[1]!="Y" | colnames(Sample)[2]!= "E" | colnames(Sample)[3] != "G"){
          ...
          stats <- t(as.matrix(run.BMA.1DF(Y=Y,E=E,G=G,Cov=Cov,cc=cc,co=co,phi=phi,psi=psi,formul=formul)))
        ...
        if(colnames(Sample)[1]!="Y" | colnames(Sample)[2]!= "E" | colnames(Sample)[3] != "G"){
          ...
          Y<-Sample$Y;E<-Sample$E;G<-Sample$G
          stats <- t(as.matrix(run.BMA.1DF(Y=Y,E=E,G=G,Cov=NULL,cc=cc,co=co,phi=phi,psi=psi,formul=formul)))
          colnames(stats) <- c("Int.est","Int.sd","Z.score","p.value")
          return(stats)
        }
      }

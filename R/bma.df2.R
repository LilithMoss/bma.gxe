...
bma.df2 <- function(Sample=NULL,Cov=NULL,cc=0.5,co=0.5,phi=1,psi=1000,formul=NULL){
  if(cc+co!=1){
    ...
    if(length(Cov)>0){
      if(nrow(Sample)!=nrow(Cov)){
        ...
        if(colnames(Sample)[1]!="Y" | colnames(Sample)[2]!= "E" | colnames(Sample)[3] != "G"){
          ...
          pval <- run.BMA.2DF(Y=Y,E=E,G=G,Cov=Cov,cc=cc,co=co,phi=phi,psi=psi,formul=formul)
          return(pval)
        }
      }
      ...
      if(colnames(Sample)[1]!="Y" | colnames(Sample)[2]!= "E" | colnames(Sample)[3] != "G"){
        ...
        Y<-Sample$Y;E<-Sample$E;G<-Sample$G
        pval <- run.BMA.2DF(Y=Y,E=E,G=G,Cov=NULL,cc=cc,co=co,phi=phi,psi=psi,formul=formul)
        return(pval)
      }
    }
  }
